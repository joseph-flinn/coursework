...
class TestLogin(ViewTestMixin):
    ...
    def test_login_failure(self):
        ...
        response = self.login(identity='foo@bar.ccom')
        assert_status_with_message(200, response,
...
class TestPasswordReset(ViewTestMixin):
    ...
    def test_begin_password_reset_as_logged_in(self):
        """ Being password reset should redirect to settings. """
        self.login()
        response = self.client.get(url_for('user.begin_password_reset'),
    ...
    def test_password_reset_empty_token(self):
        ...
        reset = {'password': 'newpassword', 'confirm_password': 'newpassword'}
        response = self.client.post(url_for('user.password_reset'), data=reset,
                                    follow_redirects=True)
    ...
    def test_reset_password_confirm_password_invalid(self):
        """ Signup Failure due to unequal confirm passwords. """
        user = {
        ...
        response = self.client.post(url_for('user.password_reset'),
                                    data=user, follow_redirects=False)
...
class TestSignup(ViewTestMixin):
    ...
    def test_signup_confirm_password_invalid(self):
        """ Signup Failure due to unequal confirm passwords. """
        user = {
            'email': 'admin@local.host',
            'password': 'newpassword',
    ...
    def test_welcome(self, users):
        ...
        assert_status_with_message(200, response,
                                   'Sign up is complete, enjoy our service.')
...
class TestUpdateCredentials(ViewTestMixin):
    ...
    def test_begin_udpate_credentials_existing_email(self):
    ...
    def test_begin_update_credentials_email_password(self):
        """ Update both the email and the new password. """
        self.login()
    ...
    @pytest.mark.skip(reason='Somehow the confirm_passwor is not working')
    def test_begin_update_credentials_confirm_password_invalid(self):
        """ Test unequal confirm passwords. """
        self.login()
